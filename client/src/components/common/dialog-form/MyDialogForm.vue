...
import RichTextEditor from '@/components/common/rich-text-editor/RichTextEditor.vue';
              ...
              <v-row>
                <v-col v-for="item in formField" :key="item.label" :cols="item.cols">
                  ...
                  <template v-if="item.inputType === FormInputType.RICH_TEXT_EDITOR">
                    <vee-validation-field :name="item.name" v-slot="{ handleChange }">
                      <rich-text-editor :handleContentChange="handleChange" />
                    </vee-validation-field>
                  </template>
